...
const WebsiteSetting = require("../models/website-setting-model");
...
const getProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.userId).select("-password -totpSecret -totpQrCode -emailOtp -passwordResetOtp -refreshTokens");
...
if (req.file) {
  if (oldUser.profilePic) {
    await deleteFromCloudinary(oldUser.profilePic);
  }
  ...
  filteredUpdateData.profilePic = imageUrl;
}
...
const updateWebsiteSetting = async (req, res) => {
  try {
...
if (req.files?.favicon?.[0]) {
  const faviconUrl = await uploadToCloudinary(
...
const getWebsiteSetting = async (req, res) => {
  try {
...
module.exports = { getProfile, updateProfile, updatePassword ,updateWebsiteSetting,getWebsiteSetting};